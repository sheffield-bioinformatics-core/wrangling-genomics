#!/usr/bin/env bash
#SBATCH -J variant_calling_batch_array_job
#SBATCH -N 1
#SBATCH -n 1
#SBATCH --array=1-3
#SBATCH --mem=6G
...
# Extract the nth line of file_list.txt
# and assign that to variable fq1.
# NB file_list.txt was created with:
#    ls ~/dc_workshop/data/trimmed_fastq/*_R1.trim.fq.gz > file_list.txt
fq1=$(awk "NR==${SLURM_ARRAY_TASK_ID}" file_list.txt)

genome=~/dc_workshop/data/ref_genome/chr20.fa

cd ~/dc_workshop/results

# The following steps should have been done _before_ running this array job:
#    bwa index $genome
